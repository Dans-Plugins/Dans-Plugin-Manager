...
copy_latest_plugin_jar() {
    ...
    local jarFile=$(find "$BUILD_DIR/DansPluginManager/target" -maxdepth 1 -name "DansPluginManager-*.jar" -not -name "original-*" -type f -print -quit)
...
start_server() {
    ...
    log "To reload Dans Plugin Manager after rebuilding, use: /serverutils reload DansPluginManager"
    log "To unload: /serverutils unload DansPluginManager"
    log "To load: /serverutils load DansPluginManager"
