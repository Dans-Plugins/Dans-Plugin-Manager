...
### Added
- `/dpm list outdated` — a read-only view of which installed plugins are behind the newest build on their pinned channel, shown as `<name> <installed> → <latest>`. It makes the same comparison `/dpm update` makes before downloading, but never downloads or changes anything, so deciding whether to update is separable from updating. Plugins whose status cannot be determined (no published build, or GitHub unreachable) are listed separately rather than counted as up to date. Tab completion for `/dpm list` now offers `outdated` (#125)
- A console warning on every startup and `/dpm reload` while `githubToken` is empty, naming the 60-requests-per-hour unauthenticated limit and how to raise it. Before this the only sign that the token was missing came later, when a rate-limited lookup surfaced as "could not reach GitHub" partway through an update; the rate-limit response itself has been reported distinctly from a network failure since #90 (#126)
...
- Introduced `CleanController` (`controllers/` package), which now owns `/dpm clean`'s duplicate-JAR detection and removal, returning plain result data (`CleanController.Conflict`/`CleanResult`) instead of calling `sender.sendMessage()` directly. `CleanCommand` is now a thin router that formats and sends the results; the async dispatch pattern is unchanged. No user-visible behavior change (#110)
- Introduced `ReloadController` (`controllers/` package), which now owns `/dpm reload`'s config re-read, GitHub token and experimental-tag application, and release-cache invalidation. `ReloadCommand` is now a thin router that calls the controller and reports the outcome, and `DansPluginManager.reloadDpm()` has been removed in favour of it. Plugin startup now applies the same config-backed settings through `ReloadController.applySettings()` rather than repeating them inline, so the startup and reload paths cannot drift apart. No user-visible behavior change (#115)
- Completes the controller layer proposed in #94 — every command listed in that issue's controller table now has one

### Fixed
- Results of `/dpm get`, `/dpm update`, `/dpm info`, and `/dpm clean` are no longer lost when the command is sent over RCON. These commands finish in the background, after an RCON client has already received its reply, and the result was sent only to that client — so it appeared nowhere. Results for any sender other than the console are now also written to the server log, prefixed with `[to <sender>]` (#124)
- Installing a plugin that is already present under a different filename no longer leaves both JARs in the plugins folder. Two JARs declaring the same plugin is a name Bukkit refuses, so the server broke at the *next* restart — one restart after the command that caused it. Two gaps allowed it: the managed-file exclusion compared filenames case-insensitively, so `Herald.jar` was skipped as the canonical copy of `herald.jar` while the install wrote the latter beside it; and the filename heuristic only stripped version suffixes, so a build qualifier such as `WildPets-ci-build.jar` matched nothing. Conflicts are now decided by the `name:` a JAR declares in its own `plugin.yml` — the field Bukkit itself uses — with the filename heuristic kept as the fallback for a JAR that cannot be read, and the managed-file comparison is now exact (#130)
...
- The two unit tests that make a directory read-only (`RemoveControllerTest.remove_returnsDeleteFailedOutcomeWhenFileCannotBeDeleted`, `DownloadServiceTest.downloadLatest_returnsFileError_whenDestinationUnwritable`) no longer fail when the suite runs as root. Their guard checked that `setWritable(false)` returned true, which it does for root even though root then bypasses the mode bits; it now probes whether a write into the directory is actually refused and skips the test when it is not. CI, which does not run as root, is unaffected (#120)
...
- Introduced `ListController` (`controllers/` package), which now owns `/dpm list`'s installed-plugin listing and version lookup for the all/installed/available modes, returning plain result data (`ListController.ListEntry`) instead of calling `sender.sendMessage()` directly. `ListCommand` is now a thin router that formats and sends the results. No user-visible behavior change (#113).
- Introduced `InfoController` (`controllers/` package), which now owns `/dpm info`'s record lookup, release-metadata fetch, and install-status resolution for a plugin and its declared dependencies, returning plain result data (`InfoController.PluginInfo`) instead of calling `sender.sendMessage()` directly. `InfoCommand` is now a thin router that formats and sends the results; the plugins-folder scan backing the dependency install status now runs inside the existing async task rather than on the main thread. No user-visible behavior change (#112).
